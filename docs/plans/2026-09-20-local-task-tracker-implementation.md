...
**Tech Stack:** Java 21, Maven, Spring Boot 4.1.1, Spring AI 2.0.1, Temporal Java SDK 1.36.1 / server 1.32.0, PostgreSQL, Flyway, JUnit, Docker, Ollama.
...
Use one Maven application with package root `com.brianmehrman.harness`. Package by responsibility. `src/main/java/com/brianmehrman/harness/` is abbreviated `J/` below, and `src/test/java/com/brianmehrman/harness/` is `T/`. These abbreviations always expand to those paths; they are not actual directories. Keep each public Java type in its matching file.
...
**Files:** Create `pom.xml`, `mvnw`, `mvnw.cmd`, `.mvn/wrapper/maven-wrapper.properties`, `.gitignore`, `compose.yaml`, `config/images.lock`, `J/HarnessApplication.java`, `J/config/TemporalConfiguration.java`, `src/main/resources/application.yaml`, `src/main/resources/db/migration/V1__create_model_profiles.sql`, `T/config/InfrastructureIT.java`, `docs/development.md`.
...
- [ ] Add Maven wrapper 3.9.16, the selected Boot parent, Java release 21, Spring AI BOM 2.0.1, Temporal SDK/testing 1.36.1, JDBC/Flyway/PostgreSQL, validation, actuator, JUnit and integration-test support. Add Surefire unit and Failsafe `*IT` execution with required checks on test discovery. Use direct SDK beans rather than a Temporal Boot starter.
...
- [ ] Run `./mvnw -Dit.test=InfrastructureIT verify`; observe failure attributable to the missing application/service wiring, not a silently skipped test.
...
- [ ] Run `docker compose up -d postgres temporal` and `./mvnw -Dit.test=InfrastructureIT verify`. Record effective dependency versions and image digests in `docs/development.md`, including model inference not yet tested. Verify Flyway's description is `create model profiles`.
...
**Files:** Create `J/workspace/{SnapshotRef,WriteRequest,WorkspaceStore,JdbcWorkspaceStore,WorkspacePathPolicy,SnapshotHasher}.java`, `src/main/resources/db/migration/V2__create_workspace_snapshots.sql`, `benchmarks/task-tracker-v1/starter/{pom.xml,REQUIREMENTS.md,src/main/java/example/tasktracker/TaskTracker.java}`, `T/workspace/{WorkspacePathPolicyTest,WorkspaceStoreIT}.java`.
...
- [ ] Run `./mvnw -Dtest=WorkspacePathPolicyTest -Dit.test=WorkspaceStoreIT verify`; confirm the missing implementation fails.
...
- [ ] Run `./mvnw -Dit.test=RunnerIT verify`; observe expected missing-runner failures.
...
- [ ] Bake trusted Maven/plugins/dependencies into the pinned image. Fixed BUILD is `mvn -o -B -ntp -DskipTests package`; TEST is `mvn -o -B -ntp test`. All Maven flags/build inputs are runner-owned. Use separate build/test copies, temporary bounded writable directories and no host mounts. Preserve candidate test execution, but do not trust its XML for benchmark verdicts.
...
- [ ] Run `./mvnw -Dit.test=TaskTrackerEvaluatorIT verify`; confirm the evaluator does not yet exist/pass.
...
- [ ] Run `./mvnw -Dtest=OllamaAdapterTest,ProfileProbeTest test`; confirm red behavior before adapter code.
...
- [ ] Run `./mvnw -Dit.test=RunCommandServiceIT,CommandDeliveryIT verify` before service implementation.
...
- [ ] Run `./mvnw -Dtest=CodingWorkflowTest,ToolRouterTest test`; inspect the expected failures.
...
- [ ] Run `./mvnw -Dit.test=RunApiIT -Dtest=ApiSecurityTest verify` and observe failures before controller implementation.
...
- [ ] Run `./mvnw -Dit.test=RecoveryIT,DeploymentIT verify`; confirm tests fail without the required configuration/behavior. Do not emulate a crash by throwing an exception in the same JVM.
...
- [ ] Add opt-in `live-local` Maven profile and test requiring an explicitly selected installed model. Normal CI runs no paid/remote inference and does not silently count skipped live acceptance as a pass. Missing local prerequisites produce a setup failure in an explicitly requested live run.
...
- [ ] Run `./mvnw verify`, the recovery/deployment scripts, and the explicitly configured live-local suite appropriate to the final changes. Update documentation with actual evidence. Do not repeat inference without a changed hypothesis or validation need.
